...
/proc/curse_of_madness(mob/user, message)
	...
	deadchat_broadcast("A <span class='name'>Curse of Madness</span> has stricken the sector, shattering their minds with the awful secret: \"<span class='big hypnophrase'>[message]</span>\"", message_type=DEADCHAT_ANNOUNCEMENT)
...
/proc/give_madness(mob/living/carbon/human/H, message)
	...
	switch(rand(1,10))
		if(1 to 3)
			H.gain_trauma_type(BRAIN_TRAUMA_MILD, TRAUMA_RESILIENCE_CEREBRAL_RESECTION)
			H.gain_trauma_type(BRAIN_TRAUMA_MILD, TRAUMA_RESILIENCE_CEREBRAL_RESECTION)
		if(4 to 6)
			H.gain_trauma_type(BRAIN_TRAUMA_SEVERE, TRAUMA_RESILIENCE_CEREBRAL_RESECTION)
		if(7 to 8)
			H.gain_trauma_type(BRAIN_TRAUMA_MAGIC, TRAUMA_RESILIENCE_CEREBRAL_RESECTION)
		if(9 to 10)
			H.gain_trauma_type(BRAIN_TRAUMA_SPECIAL, TRAUMA_RESILIENCE_CEREBRAL_RESECTION)
